#!/usr/bin/env python3
"""Develop screen for oil_session_scalp_develop_v1.

Lock first. Rank develop only. Holdout unused for selection.
96 configs (3 families x 2 sl x 2 tp x 2 ts x 2 eia x 2 atr).
...
from oil_session_scalp_backtest import (  # noqa: E402
    LOCK_PATH,
    costs_from_lock,
    holdout_start_from_lock,
    load_lock,
    pack_slices,
    require_oil_cost_book,
    run_family,
    run_transfers,
    simulate_exits,
    split_by_holdout,
...
def iter_grid(lock: dict) -> list[dict]:
    ...
    eia_key = "eia_blackout" if "eia_blackout" in g else "nfp_blackout"
    rows = []
    for fam, sl, tp, ts, eia, atr in itertools.product(
        fams,
        g["sl_atr"],
        g["tp_r"],
        g["time_stop_bars"],
...
def cfg_key(c: dict) -> str:
    return (
        ...
        f"|eia{int(c['eia_blackout'])}|atr{c['min_atr_pct']}"
    )
...
def run_screen(lock: dict) -> dict:
    ...
    def _sigs(fam: str, min_atr: float, eia: bool):
        key = (fam, float(min_atr), bool(eia))
        if key not in sig_cache:
            sig_cache[key] = family_signals(
                d, fam, min_atr_pct=float(min_atr), eia_blackout=bool(eia)
            )
        return sig_cache[key]

    rows = []
    for c in grid:
        sigs = _sigs(c["family"], c["min_atr_pct"], c["eia_blackout"])
        clock, fmin = flatten_spec_for_family(c["family"])
    ...
    null_scores = []
    nclock, nfmin = flatten_spec_for_family(null_cfg["family"])
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument(
        "--out",
        type=Path,
        default=_ROOT / "results" / "oil_session_scalp_autoresearch.json",
    )
    p.add_argument(
        "--md",
        type=Path,
        default=_ROOT / "results" / "oil_session_scalp_autoresearch.md",
    )
    args = p.parse_args(argv)
    lock = load_lock(args.lock)
    if lock.get("search_id") != "oil_session_scalp_develop_v1":
