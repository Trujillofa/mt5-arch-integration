...
Integrity rules (v6):
  ...
  * Full H1 is stored; develop windows are derived from holdout cutoff (no duplicate CSV).
...
# Package stores full H1 only; develop is derived (holdout filter) unless opted in.
STORE_DEVELOP_CSV = False
DEVELOP_DERIVATION = "server_time < holdout_start_server"
...
def _develop_from_h1(h1: pd.DataFrame, holdout_start: pd.Timestamp) -> pd.DataFrame:
    """Deterministic develop slice: server_time < holdout_start (naive)."""
    if h1.empty:
        return h1.copy()
    tcol = h1["time"]
    if not pd.api.types.is_datetime64_any_dtype(tcol):
        tcol = pd.to_datetime(tcol)
...
def build_symbol(
    ...
    *,
    ...
    if not hard and publish:
        ...
        # Develop is derived from full H1 + holdout (default: do not store duplicate CSV).
        develop_sha = _sha256_develop_frame(develop[cols] if len(develop) else develop)
        if STORE_DEVELOP_CSV:
            ...
            develop_rel = f"derived:{DEVELOP_DERIVATION}"
...
def write_report(
    ...
    *,
    ...
    lines = [
        "# Multi-instrument data readiness (Phase 0 — integrity v6.1 pipeline)",
        "",
...
def write_artifact_lock(
    ...
    *,
    ...
    for m in manifests:
        ...
        # Develop: prefer stored CSV; else use manifest-derived hash/count.
        if STORE_DEVELOP_CSV and develop_path.is_file():
            develop_sha = _sha256_file(develop_path)
            n_dev = sum(1 for _ in develop_path.open()) - 1
            develop_rel = f"{path_prefix}/{develop_name}"
            develop_mode = "stored_csv"
        else:
            dsha = m.missing_duplicate_bars.get("develop_csv_sha256")
            develop_sha = str(dsha) if dsha else ""
            n_dev = int(m.n_rows_h1_develop)
            develop_rel = f"derived:{DEVELOP_DERIVATION}"
    ...
    for sym in sorted(required):
        ...
        if not drel:
            ...
            if not rel:
                ...
                if not rp.is_file():
                    errs.append(f"{sym}:full_file_missing_for_develop:{rel}")
                else:
...
def _required_package_rels() -> list[str]:
    ...
    rels = [f"instrument_data/{s.lower()}_h1.csv" for s in SYMBOLS]
    if STORE_DEVELOP_CSV:
        rels += [f"instrument_data/{s.lower()}_h1_develop.csv" for s in SYMBOLS]
    rels += [f"instrument_data_manifests/{s.lower()}_h1_manifest.json" for s in SYMBOLS]
    rels += [
...
def _validate_lock_schema(lock: dict[str, Any]) -> list[str]:
    ...
    for sym in SYMBOLS:
        ...
        want_dev_stored = f"{EXPECTED_PATH_PREFIX}/{sym.lower()}_h1_develop.csv"
        want_dev_derived = f"derived:{DEVELOP_DERIVATION}"
        if ent.get("research_csv") != want_full:
            errs.append(
                f"{sym}:LOCK_PATH_FULL:have={ent.get('research_csv')!r} need={want_full!r}"
            )
        dpath = ent.get("develop_csv")
        if STORE_DEVELOP_CSV:
            if dpath != want_dev_stored:
                errs.append(
                    f"{sym}:LOCK_PATH_DEVELOP:have={dpath!r} need={want_dev_stored!r}"
                )
        else:
            if dpath != want_dev_derived:
                errs.append(
                    f"{sym}:LOCK_PATH_DEVELOP:have={dpath!r} need={want_dev_derived!r}"
                )
    return errs
...
class PackageSnapshot:
    ...
    def develop_is_derived(self) -> bool:
        return not STORE_DEVELOP_CSV
    ...
    def read_develop(
        ...
        *,
        ...
        """Load develop window for symbol from pinned package.

        Default package layout stores full H1 only; develop is filtered by
        ``holdout_start`` (default DEVELOP_END_SERVER).
        """
        path = self.develop_csv(symbol)
        if STORE_DEVELOP_CSV and path.is_file():
            return pd.read_csv(path)
        # Derived from full H1
        full = self.read_history(symbol)
        if "time" in full.columns:
            full = full.copy()
            full["time"] = pd.to_datetime(full["time"])
        hs = holdout_start if holdout_start is not None else DEVELOP_END_SERVER
        return _develop_from_h1(full, hs)
