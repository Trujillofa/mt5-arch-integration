...
def read_mt5_hc_m5(path: Path) -> pd.DataFrame:
    """Read a Wine MT5 ``cache/M5.hc`` bar cache (columnar, count-prefixed).
    ...
    for off in range(128, 720, 8):
        ...
        if lo <= v0 <= hi and 60 <= (v1 - v0) <= 3600:
            start = off
            break
    if start < 0:
        raise ValueError(f"no M5 time table in {path}")
    times: list[int] = []
    off = start
...
def hc_to_export_csv(hc_path: Path, csv_path: Path, symbol: str) -> Path:
    df = read_mt5_hc_m5(hc_path)
