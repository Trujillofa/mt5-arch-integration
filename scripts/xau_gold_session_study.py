...
WINDOW (research invariant, results/xau_holdout_lock.json)
  --window develop  (default) trading days before holdout_start. Explore here.
                    This is the only window this study accepts.
  --window holdout  refused. This study has no freeze charter, so it must not
                    read the sealed holdout. results/xau_holdout_lock.json is
                    not a selection set.
  --window all      refused. Mixing develop and holdout is not a charter.
                    Do not retune. promote=no.
...
EXAMPLES
  ...
  --window holdout and --window all are refused. This study has no freeze
  charter. results/xau_holdout_lock.json is sealed and is not a selection set.
"""
...
def load_csv(path: str, default_spread: float) -> pd.DataFrame:
    ...
    if "date" in raw.columns and "time" in raw.columns:
        ts = pd.to_datetime(
            raw["date"].astype(str).str.replace(".", "-", regex=False)
            + " "
            + raw["time"].astype(str)
        )
...
def server_to_utc(ts: pd.Series, mode: str) -> pd.Series:
    if mode == "ny-close":  # server clock = New York wall clock + 7h
        ny = (ts - pd.Timedelta(hours=7)).dt.tz_localize(NY, ambiguous="NaT", nonexistent="NaT")
        return ny.dt.tz_convert("UTC")
    if mode.startswith("fixed:"):
        return (ts - pd.Timedelta(hours=float(mode.split(":", 1)[1]))).dt.tz_localize("UTC")
    return ts.dt.tz_localize(mode, ambiguous="NaT", nonexistent="NaT").dt.tz_convert("UTC")
...
def check_server_tz(csv: str, mode: str) -> None:
    ...
    if abs(want - seen) > 0.25:
        print(
            f"WARNING: --server-tz {mode} implies UTC{want:+g}h at {at}, but the terminal "
...
def cost_sensitivity_points() -> list[float]:
    try:
        return [
            float(x)
            for x in json.loads(COSTS_LOCK.read_text())["slippage_sensitivity_points_frozen"]
        ]
...
def prepare(df: pd.DataFrame, a) -> pd.DataFrame:
    ...
    df = df.dropna(subset=["utc"]).drop_duplicates("utc").sort_values("utc").reset_index(drop=True)
    ...
    df["regime"] = np.select([us_dst & uk_dst, ~us_dst & ~uk_dst], ["summer", "winter"], "mismatch")
    ...
    m15 = (
        df.set_index("utc")[["high", "low", "close"]]
        .resample("15min")
    ...
    tr = pd.concat(
        [m15["high"] - m15["low"], (m15["high"] - prev).abs(), (m15["low"] - prev).abs()], axis=1
    ).max(axis=1)
    atr = tr.ewm(alpha=1 / 14, adjust=False).mean()
    atr_df = pd.DataFrame({"t": atr.index + pd.Timedelta(minutes=15), "atr15": atr.to_numpy()})
...
def simulate(side, entry, stop, target, op, hi, lo, cl, sp, be_trigger=None):
    ...
    for t in range(len(hi)):
        ...
        if hit_stop:
            return (
                side * (fill - entry) / risk,
                ("be" if moved else "stop"),
                mfe / risk,
                mae / risk,
                t + 1,
            )
            ...
            out.append(
                {
                    ...
                    "real_share": real[b] / n,
                    "null_share": mean[b] / n,
                    "excess": (real[b] - mean[b]) / n,
                    "z": z[b],
                    "p": p_upper(real[b], null[:, b]),
                }
            )
...
def report_timing(days, stats, a):
    ...
    print(
        f"days: {len(days)}  {cnt}   up: {(days['dir'] == 'up').sum()}  "
    ...
    print(f"{'clock':<10}{'summer':>20}{'winter':>20}{'shift':>8}{'mismatch':>20}{'all days':>22}")
    ...
    for r in rows:
        ...
        print(
            f"  {r['regime']:<7} {r['real_share'] * 100:5.1f}% of days vs "
    ...
    for k in CLOCKS:
        ...
        cells = [
            f"{r.bucket} {r.real_share * 100:.1f}% (null {r.null_share * 100:.1f}%, p {r.p:.3f})"
    ...
    for tday, g in trading_days(df, a.min_bars):
        ...
        for name, (s, e) in refs.items():
            ...
            for side in (+1, -1):
                ...
                rec = {
                    "tday": tday.date(),
                    "regime": g["regime"].iat[0],
                    "ref": name,
                    "side": "long@low" if side > 0 else "short@high",
                    "level": level,
                }
                ...
                risk = abs(entry - stop)
                rec.update(
                    entry_utc=fmt(g["utc_min"].iat[k]), entry=entry, stop=stop, target=vw, risk=risk
                )
...
def fade_study(df, a):
    ...
    for tday, g in trading_days(df, a.min_bars):
        ...
        for label, clock, m in anchors:
            ...
            risk = abs(entry - stop)
            rec.update(
                side="long" if side > 0 else "short", entry=entry, stop=stop, target=vw, risk=risk
            )
            if (vw - entry) * side <= 0:
                out.append({**rec, "status": "vwap_behind"})
                continue
...
def build_parser():
    ...
    ap.add_argument(
        "--server-tz", default="ny-close", help="ny-close | fixed:+2 | IANA zone (default ny-close)"
    )
    ...
    ap.add_argument(
        "--null-block", type=int, default=30, help="minutes per sign-flip block in the null"
    )
    ap.add_argument(
        "--claim",
        default="15:30-16:30@utc",
        help="pre-stated extreme window to test, HH:MM-HH:MM@utc|lon|et",
    )
    ap.add_argument(
        "--ref-windows",
        default="LDN_AM=08:00-13:00,LDN_FULL=08:00-16:00",
    ...
    ap.add_argument(
        "--ext-atr", type=float, default=1.0, help="fade only if |price - VWAP| >= this x ATR(M15)"
    )
    ap.add_argument(
        "--stop-lookback", type=int, default=60, help="fade stop beyond this many minutes' extreme"
    )
...
def refuse_unfrozen_holdout(window: str) -> None:
    """Holdout is sealed. This study has no freeze, so it must not read it."""
    if window not in ("holdout", "all"):
        return
    sys.exit(
        f"refusing --window {window}: xau_gold_session_study has no freeze charter. "
        "results/xau_holdout_lock.json is sealed and is never used for selection. "
        "Do not retune. promote=no."
    )
...
def main(argv=None):
    ...
    refuse_unfrozen_holdout(a.window)
    check_server_tz(a.csv, a.server_tz)
    df = prepare(load_csv(a.csv, a.default_spread), a)
    print(
        f"bars: {len(df):,}  {df['utc'].iat[0]:%Y-%m-%d} -> {df['utc'].iat[-1]:%Y-%m-%d} UTC"
    ...
    if a.window == "all":
        print(
            "WARNING: --window all mixes develop and holdout days; nothing here is out of sample."
        )
    ...
    if days.empty:
        sys.exit(
            f"No complete trading days in the {a.window} window. Check the CSV range, "
    ...
    fd = fade_study(df, a)
    f_sum = report_trades(
        f"3) FADE EXTENSION >= {a.ext_atr:g} ATR(M15) FROM LONDON VWAP", fd, ["anchor"], a
    )
