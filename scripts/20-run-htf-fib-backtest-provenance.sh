...
if [[ "$SKIP_TESTER" == "1" ]]; then
  ...
  info "Calling scripts/19-run-htf-fib-backtest.sh $RESOLVED $PERIOD $FROM $TO (run_start=$RUN_START)"
  set +e
...
OUT="$(
  ...
  PROVENANCE_OUT="$PROVENANCE_OUT" RUN_START="$RUN_START" SKIP_TESTER="$SKIP_TESTER" python3 - <<'PY'
import os, sys
from pathlib import Path
...
run_start = int(os.environ.get("RUN_START") or "0")
executed = os.environ.get("SKIP_TESTER") != "1"
report = (
    find_latest_report(
        mt5 / "reports",
        resolved,
        period,
        after_mtime=float(run_start) if executed else None,
...
record_tester_run(
    ...
    run_started_at=run_start or None,
)
print(out)
PY
)"
