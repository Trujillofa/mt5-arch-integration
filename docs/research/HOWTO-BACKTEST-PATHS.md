...
| XAU session scalp | **CLOSED** — do not rerun `xau_session_scalp_defined_r.py` for official metrics. Historical: `python3 scripts/xau_session_scalp_backtest.py --mode all` | Terminal record: `tpr1_be0_ts6` eligible=false. Holdout unused. **Do not retune.** | Gold book frozen in `defined_r_v1_lock.json`. | [XAU-SESSION-SCALP.md](XAU-SESSION-SCALP.md) · [STATUS.md](../../results/xau_session_scalp/STATUS.md). Overlay observe-only. |
