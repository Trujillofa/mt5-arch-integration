...
**Status:** product decision draft (not yet adopted)  
**Date:** 2026-08-08  
**Context branch:** `research/algo-trading-btc-gold-forex`  
**Current XAU disposition:** `RESEARCH_IDLE` · `promote=no` · `live_go=false`  
**Related:** [`AGENTS.md`](../AGENTS.md), [`CLAUDE.md`](../CLAUDE.md), [`results/xau_loop_status.md`](../results/xau_loop_status.md)

This note records three options for how offline strategy research relates to the
`mt5-arch-integration` platform charter. It does **not** authorize live trading,
parameter promotion, or a merge by itself — choose an option, then act.
...
Hard rules that apply under **any** option:
...
### Adopted defaults until a human picks otherwise

Until an explicit product decision overwrites this file:
...
| Auto-open merge PR | **No** — draft only; human chooses A/B/C first if disputed |
...
| — | Official adoption | TBD |

When adopted, update `AGENTS.md` “Two layers” section to link here and set
**Status** at the top of this file to `adopted` or `rejected-for-A` / etc.
