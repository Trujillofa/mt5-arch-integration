...
returns 4002 on Vantage 6090). The exporter deletes any leftover
`MQL5/Files/mt5_arch/htf_fib_effective_<SYMBOL>.txt` before `iCustom` and
refuses unless `OnInit` recreates it for this run. The sidecar must name the
scan symbol and chart TF. The exporter copies those values into the manifest
(`indicator_left` / `indicator_right` / `indicator_fib_source` /
`indicator_version` / `indicator_symbol` / `indicator_chart_tf`). For
`source=mql5_export` the verifier requires them, asserts left/right/source
equal the scan and the HTF mapping (H4→0, D1→1), and compares
`indicator_version` to the repo `#define HTF_FIB_VER` (a stale or bogus
string fails). The pin catches a mismatched **scan** (exporter `InpLeft` ≠
indicator defaults) and a leftover sidecar from another symbol/TF/build.
...
| Config pin | sidecar rewritten after iCustom; symbol/TF/version | must equal scan `left`/`right`, HTF map, and repo `HTF_FIB_VER` | leftover sidecar or stale version |
