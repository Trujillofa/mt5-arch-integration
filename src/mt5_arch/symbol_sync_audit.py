...
def verify_sync_audit_dump(
    ...
    for row in rows:
        ...
        if stamps:
            ...
            if len(stamps) != bars:
                errors.append(
                    f"{mapping.canonical} len(timestamps) {len(stamps)} != bars_h1 {bars}"
                )
            if first and last:
            ...
            missing_ts.append(mapping.canonical)
    ...
    if (
        ...
        errors.append(
            "equal bar counts across "
            + ", ".join(sorted(counts))
            + " without timestamps (refuse identity expansion)"
        )

    inter: set[str] = set()
    if closed_sets:
        inter = set.intersection(*closed_sets.values()) if closed_sets else set()
