...
def report_name_prefix(symbol: str, period: str) -> str:
    return f"htf_fib_{symbol}_{period}_"
...
def find_latest_report(
    ...
    *,
    ...
    """Return the newest exact symbol/period report newer than ``after_mtime``.

    Executed runs must pass a run-start boundary and must not fall back to
    another symbol's report (HIGH-2).
    """
    if not reports_dir.is_dir():
        return None
    prefix = report_name_prefix(symbol, period)
    cands: list[Path] = []
    for path in reports_dir.iterdir():
        name = path.name
        if name.endswith(".provenance.json") or not path.is_file():
            continue
        if name.startswith(prefix):
            cands.append(path)
    if not cands and allow_cross_symbol:
        cands = [
            p
    ...
    if after_mtime is not None:
        cands = [p for p in cands if p.stat().st_mtime > after_mtime]
    if not cands:
        return None
...
def verify_provenance(
    ...
    period = str(tester.get("period") or "").strip()
    report_name_sym = broker_symbol or requested
    if source == "mql5_export":
        if not report_raw:
            ...
            if not report.exists():
                ...
                prefix = report_name_prefix(report_name_sym, period) if period else ""
                if prefix and not report.name.startswith(prefix):
                    errors.append(
                        f"report name {report.name!r} is not {prefix}* "
                        "(wrong symbol/period; no cross-symbol fallback)"
                    )
                ident = raw.get("report") if isinstance(raw.get("report"), dict) else {}
                live = report_identity(report)
                if not ident:
                    errors.append(
                        "report identity (sha256/size/mtime) is required when source=mql5_export"
                    )
                else:
                    if not is_sha256_hex(ident.get("sha256")):
                        errors.append("report.sha256 missing or invalid")
                    elif ident.get("sha256") != live["sha256"]:
                        errors.append("report sha256 does not match file (tampered or stale)")
                    if ident.get("size") != live["size"]:
                        errors.append(
                            f"report size {ident.get('size')} != file {live['size']}"
                        )
                    recorded_mtime = ident.get("mtime")
                    if recorded_mtime is None:
                        errors.append("report.mtime is required")
                    elif int(recorded_mtime) != int(live["mtime"]):
                        errors.append(
                            f"report mtime {recorded_mtime} != file {live['mtime']}"
                        )
                    ...
                    if started is not None:
                        try:
                            ...
                            start_i = None
                            errors.append("run_started_at must be a unix epoch")
                        if start_i is not None and int(live["mtime"]) <= start_i:
                            errors.append(
                                f"report mtime {live['mtime']} is not newer than "
                                f"run_started_at {start_i} (stale report)"
                            )
...
def build_provenance(
    *,
    ...
    run_started_at: int | float | None = None,
) -> dict[str, Any]:
    ...
    return {
        ...
        "run_started_at": int(run_started_at) if run_started_at is not None else None,
        "report": (
            report_identity(Path(report_path))
