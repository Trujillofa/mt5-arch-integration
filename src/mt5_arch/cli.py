...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="mt5-arch",
        description="MetaTrader 5 tools for Arch Linux (Wine + mt5linux/RPyC)",
    )
...
def cmd_ping(client: MT5ArchClient, as_json: bool) -> int:
    info = client.ping()
    payload = asdict(info)
...
def cmd_account(client: MT5ArchClient, as_json: bool) -> int:
    account = client.account_info()
    _print_result(asdict(account), as_json=as_json)
    return 0


def cmd_symbols(client: MT5ArchClient, symbols: Sequence[str], as_json: bool) -> int:
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        with MT5ArchClient(settings) as client:
        ...
        print(
            ...
            "hint: ensure Wine MT5 is running and mt5server is listening on "
            f"{settings.mt5_rpyc_host}:{settings.mt5_rpyc_port}",
            file=sys.stderr,
        )
        return 1
