# XAU family null / max-stat — `multi_day_variance_expansion_flat`
...
- Family: `multi_day_variance_expansion_flat` (source=xau_family_multi_day_variance_expansion_flat)
...
- Classic gates (from charter if provided): n>=40, PF>1.5, WR>55.0, DD<10.0
- Soft gates (from charter if provided): n>=40, PF>=1.2, DD<=15.0, NP>0.0
...
| max PF (n≥20) | 0.9206 |
| max net (n≥20) | $-505.68 |
| max PF raw (incl. thin) | 0.9206 |
| n_passers (primary=soft) | 0 |
| n_passers_classic | 0 |
| n_passers_soft | 0 |
| n with ≥20 trades | 1 |
| PF p50 / p90 / p99 | 0.921 / 0.921 / 0.921 |
| elapsed | 0s |
...
```json
{
  ...
  "profit_factor": 0.9206196215586736,
  "net_profit": -505.6799999999156,
  "win_rate": 46.601941747572816,
  "max_drawdown_pct": 14.893652044750485,
  "n_trades": 206,
  "expectancy": -2.4547572815529883,
...
- Fail (`KILL_MULTI_DAY_VARIANCE_EXPANSION_FLAT`) if `p_max_pf > 0.05` **or** `p_n_passers > 0.05` —
