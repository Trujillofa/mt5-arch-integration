...
import oil_session_scalp_autoresearch as ar  # noqa: E402
...
def test_grid_is_96_and_under_192():
    ...
    assert "btc_transfer" not in fams
    assert "eurusd_mr_transfer" not in fams
...
def test_closed_theses_are_in_the_lock():
    ...
    assert "CL=F" in closed
    assert "M15.hc" in closed
    assert LOCK["search_id"] == "oil_session_scalp_develop_v1"
