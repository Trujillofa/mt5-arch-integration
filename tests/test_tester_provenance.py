...
def test_wrapper_calls_19_and_keeps_secrets_out():
    ...
    assert "after_mtime" in text
    assert "allow_cross_symbol=False" in text
...
def test_build_and_verify_round_trip(tmp_path: Path):
    ...
    for name, data in blobs.items():
        ...
        p.write_bytes(data)
        paths[name] = p
    report = tmp_path / "htf_fib_XAUUSD_H1_synthetic.htm"
...
def test_stale_report_refuses(tmp_path: Path):
    reports = tmp_path / "reports"
    reports.mkdir()
    stale = reports / "htf_fib_XAUUSD_H1_old.htm"
    stale.write_text("<html>old</html>\n", encoding="utf-8")
    os.utime(stale, (1, 1))
    other = reports / "htf_fib_EURUSD_H1_new.htm"
    other.write_text("<html>other</html>\n", encoding="utf-8")
    assert find_latest_report(reports, "XAUUSD", "H1", after_mtime=10) is None
    dest, raw = _clone(tmp_path)
    raw["source"] = "mql5_export"
    raw["report_path"] = str(stale)
    raw["run_started_at"] = 100
    raw["report"] = {
        "sha256": sha256_file(stale),
        "size": stale.stat().st_size,
        "mtime": int(stale.stat().st_mtime),
    }
    dest.write_text(json.dumps(raw), encoding="utf-8")
    with pytest.raises(ProvenanceError, match="stale report"):
        verify_provenance(dest)
    stale.unlink()
    assert find_latest_report(reports, "XAUUSD", "H1") is None
    assert find_latest_report(reports, "XAUUSD", "H1", allow_cross_symbol=True) == other


def test_wrong_symbol_report_refuses(tmp_path: Path):
    dest, raw = _clone(tmp_path)
    wrong = tmp_path / "htf_fib_EURUSD_H1_x.htm"
    wrong.write_text("<html>eur</html>\n", encoding="utf-8")
    raw["source"] = "mql5_export"
    raw["report_path"] = str(wrong)
    raw["report"] = {
        "sha256": sha256_file(wrong),
        "size": wrong.stat().st_size,
        "mtime": int(wrong.stat().st_mtime),
    }
    dest.write_text(json.dumps(raw), encoding="utf-8")
    with pytest.raises(ProvenanceError, match="wrong symbol"):
        verify_provenance(dest)


def test_post_record_tamper_refuses(tmp_path: Path):
    report = tmp_path / "htf_fib_XAUUSD_H1_live.htm"
    report.write_text("<html>before</html>\n", encoding="utf-8")
    blobs = {
        "ini": b"[Tester]\r\nModel=1\r\n",
        "set": b"\xff\xfe" + "InpLots=0.01\r\n".encode("utf-16-le"),
        "mql5_expert": b"// expert\n",
    ...
    report.write_text("<html>tampered</html>\n", encoding="utf-8")
    with pytest.raises(ProvenanceError, match="tampered"):
        verify_provenance(out)
