...
import {
  ...
  fortradersBridgeCandidates,
  fortradersBridgeDir,
  readFortradersEnv,
  type FortradersOperatorEnv,
...
import type { FortradersConnectionStatus, FortradersLiveReport } from "@/lib/fortraders/types";
...
function probeMt5ArchCli(env: FortradersOperatorEnv, bridgeDir: string): Snapshot | null {
  const root =
  ...
  const result = spawnSync(uv, ["run", "mt5-arch", "account", "--json"], {
    ...
    env: {
      ...
      MT5_BACKEND: "file",
      MT5_LOGIN: env.mt5Login || FORTRADERS_EXPECTED_LOGIN,
      MT5_SERVER: env.mt5Server || FORTRADERS_EXPECTED_SERVER,
...
}): FortradersConnectionStatus {
  return deriveFileBridgeConnectionStatus({
    login: input.snapshot?.login,
    expectedLogin: FORTRADERS_EXPECTED_LOGIN,
...
export function probeFortradersLive(opts?: { poll?: boolean }): FortradersLiveReport {
  ...
  const preferredBridge = env.bridgeDir || fortradersBridgeDir(env.winePrefix);
  ...
  for (const dir of bridgeDirs) {
    ...
    if (!candidate?.login || candidate.login !== FORTRADERS_EXPECTED_LOGIN) continue;
    snapshot = candidate;
  ...
  if (snapshot?.login) {
    ...
    notes.push("No readable Fortraders account.json yet.");
    if (!opts?.poll) {
      snapshot = probeMt5ArchCli(env, preferredBridge);
      if (snapshot?.login) notes.push("mt5-arch account --json succeeded under the Fortraders prefix.");
    }
  }

  if (snapshot?.login && snapshot.login !== FORTRADERS_EXPECTED_LOGIN) {
    notes.push(
      `Refusing snapshot login ${snapshot.login} — expected ${FORTRADERS_EXPECTED_LOGIN}.`
    );
    snapshot = null;
  }
  if (
    snapshot?.server &&
    !snapshot.server.toLowerCase().includes(FORTRADERS_SERVER_NEEDLE.toLowerCase())
  ) {
    notes.push(`Refusing snapshot server ${snapshot.server} — not a Fortraders server.`);
    snapshot = null;
  }
  ...
  const bookHonesty =
    connectionStatus === "connected"
      ? `Live Fortraders MT5 ${FORTRADERS_EXPECTED_LOGIN} @ ${
          snapshot?.server || FORTRADERS_EXPECTED_SERVER
        }. Read-only file-bridge snapshot. Live OrderSend is the Fortraders live copy control.`
      : connectionStatus === "disconnected"
        ? `Fortraders ${FORTRADERS_EXPECTED_LOGIN} is not a live session — stale/missing heartbeat, terminal64 down, or trade server offline. Leftover account.json is not connected. Live OrderSend is the Fortraders live copy control.`
        : winePrefixPresent
          ? "Fortraders Wine prefix is on disk. Waiting for a fresh Mt5ArchBridge account.json (read-only EA). Live OrderSend is the Fortraders live copy control."
          : "Fortraders Wine prefix is missing. Card stays on the operator login/server; paper copy is unchanged.";
  ...
  return {
    ...
    nextSecretNeeded: liveBalance
      ? null
      : env.hasMt5Password
        ? "A running ~/.mt5-fortraders terminal with read-only Mt5ArchBridge writing account.json."
        : `FORTRADERS_MT5_PASSWORD for login ${FORTRADERS_EXPECTED_LOGIN}.`,
  };
}

export function fortradersAccountSnapshot(report: FortradersLiveReport) {
  return {
