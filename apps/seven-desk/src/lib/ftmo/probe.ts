...
import {
  ...
  FTMO_SERVER_NEEDLE,
  ftmoBridgeCandidates,
  ftmoBridgeDir,
  readFtmoEnv,
  type FtmoOperatorEnv,
} from "@/lib/ftmo/env";
import {
...
function probeMt5ArchCli(env: FtmoOperatorEnv, bridgeDir: string): Snapshot | null {
  const root =
  ...
  const result = spawnSync(uv, ["run", "mt5-arch", "account", "--json"], {
    ...
    env: {
      ...
      MT5_BACKEND: "file",
      MT5_LOGIN: env.mt5Login || FTMO_EXPECTED_LOGIN,
      MT5_SERVER: env.mt5Server || FTMO_EXPECTED_SERVER,
...
export function probeFtmoLive(): FtmoLiveReport {
  ...
  const bridgeDirs = env.bridgeDir ? [env.bridgeDir] : ftmoBridgeCandidates(env.winePrefix);
  const preferredBridge = env.bridgeDir || ftmoBridgeDir(env.winePrefix);
  ...
  for (const dir of bridgeDirs) {
    ...
    if (!candidate?.login || candidate.login !== FTMO_EXPECTED_LOGIN) continue;
    snapshot = candidate;
  ...
  if (snapshot?.login && snapshot.login !== FTMO_EXPECTED_LOGIN) {
    notes.push(
      `Refusing snapshot login ${snapshot.login} — expected ${FTMO_EXPECTED_LOGIN}.`
    );
    snapshot = null;
  }
  if (
    snapshot?.server &&
    !snapshot.server.toLowerCase().includes(FTMO_SERVER_NEEDLE.toLowerCase())
  ) {
    notes.push(`Refusing snapshot server ${snapshot.server} — not an FTMO server.`);
    snapshot = null;
  }

  const live = snapshotIsLive(snapshot);
  if (live) {
  ...
  const bookHonesty =
    connectionStatus === "connected"
      ...
      : connectionStatus === "disconnected"
        ? `FTMO ${FTMO_EXPECTED_LOGIN} is not a live session — stale/missing heartbeat, terminal64 down, or trade server offline. Leftover account.json is not connected. Live OrderSend is the FTMO live master control.`
        : winePrefixPresent
  ...
  return {
    ...
    nextSecretNeeded: live
      ? null
      : env.hasMt5Password
        ? "A running ~/.mt5-ftmo terminal with a trade-authorized session and Mt5ArchBridge writing account.json."
        : `FTMO_MT5_PASSWORD for login ${FTMO_EXPECTED_LOGIN}.`,
  };
}

export function ftmoAccountSnapshot(report: FtmoLiveReport) {
  return {
