...
function probeMt5ArchCli(env: NeomaaOperatorEnv, bridgeDir: string): Snapshot | null {
  const root =
  ...
  const result = spawnSync(uv, ["run", "mt5-arch", "account", "--json"], {
    ...
    env: {
      ...
      MT5_BACKEND: "file",
      MT5_LOGIN: env.mt5Login || NEOMAA_EXPECTED_LOGIN,
      MT5_SERVER: env.mt5Server || NEOMAA_EXPECTED_SERVER,
...
export function probeNeomaaLive(opts?: { poll?: boolean }): NeomaaLiveReport {
  ...
  for (const dir of bridgeDirs) {
    ...
    if (!candidate?.login || candidate.login !== NEOMAA_EXPECTED_LOGIN) continue;
    snapshot = candidate;
    ...
    if (!snapshotIsLive(snapshot)) {
      notes.push(
        snapshot.terminalConnected === false
          ? "Mt5ArchBridge is writing, but Neomaaa-global is disconnected (empty AccountInfo). Weekend FX — not an auth failure."
          : "Snapshot is title-only (empty currency/leverage, not trade-authorized)."
      );
    }
  } else {
    notes.push("No readable Neomaa account.json yet.");
    if (!opts?.poll) {
      snapshot = probeMt5ArchCli(env, preferredBridge);
      if (snapshot?.login) notes.push("mt5-arch account --json succeeded under the Neomaa prefix.");
    }
  }

  if (snapshot?.login && snapshot.login !== NEOMAA_EXPECTED_LOGIN) {
    notes.push(
      `Refusing snapshot login ${snapshot.login} — expected ${NEOMAA_EXPECTED_LOGIN}.`
    );
    snapshot = null;
  }
  if (
    snapshot?.server &&
    !snapshot.server.toLowerCase().includes(NEOMAA_SERVER_NEEDLE.toLowerCase())
  ) {
    notes.push(`Refusing snapshot server ${snapshot.server} — not a Neomaa server.`);
    snapshot = null;
  }
    ...
    connectionStatus === "connected"
      ...
      : connectionStatus === "disconnected"
        ? `File-bridge is live for ${NEOMAA_EXPECTED_LOGIN} @ ${
            snapshot?.server || NEOMAA_EXPECTED_SERVER
          }, but the trade server is disconnected. Balance/quotes stay empty until Neomaaa-global is up (weekend FX). Not an auth failure. Live OrderSend fail-closed; this desk will not restart ~/.mt5-neomaa or any other book. Restart Neomaa separately when Neomaaa-global is up.`
        : winePrefixPresent
  ...
  return {
    ...
    nextSecretNeeded: liveBalance
      ? null
      : connectionStatus === "disconnected"
        ? "Neomaaa-global trade-server session. Fetch Neomaa again when the journal shows authorized/connected. Restart ~/.mt5-neomaa only — do not restart other books."
        : env.hasMt5Password
...
export function neomaaAccountSnapshot(report: NeomaaLiveReport) {
  return {
