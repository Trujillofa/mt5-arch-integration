...
import {
  ...
  alphacapitalBridgeCandidates,
  alphacapitalBridgeDir,
  readAlphaCapitalEnv,
...
function probeMt5ArchCli(env: AlphaCapitalOperatorEnv, bridgeDir: string): Snapshot | null {
  const root =
  ...
  const result = spawnSync(uv, ["run", "mt5-arch", "account", "--json"], {
    ...
    env: {
      ...
      MT5_BACKEND: "file",
      MT5_LOGIN: env.mt5Login || ALPHACAPITAL_EXPECTED_LOGIN,
      MT5_SERVER: env.mt5Server || ALPHACAPITAL_EXPECTED_SERVER,
...
export function probeAlphaCapitalLive(): AlphaCapitalLiveReport {
  ...
  for (const dir of bridgeDirs) {
    ...
    if (!candidate?.login || candidate.login !== ALPHACAPITAL_EXPECTED_LOGIN) continue;
    snapshot = candidate;
  ...
  if (snapshot?.login && snapshot.login !== ALPHACAPITAL_EXPECTED_LOGIN) {
    notes.push(
      `Refusing snapshot login ${snapshot.login} — expected ${ALPHACAPITAL_EXPECTED_LOGIN}.`
    );
    snapshot = null;
  }
  if (
    snapshot?.server &&
    !snapshot.server.toLowerCase().includes(ALPHACAPITAL_SERVER_NEEDLE.toLowerCase())
  ) {
    notes.push(`Refusing snapshot server ${snapshot.server} — not an ACG Markets server.`);
    snapshot = null;
  }
  ...
  return {
    ...
    nextSecretNeeded: liveBalance
      ? null
      : env.hasMt5Password
        ? "A running ~/.mt5-alphacapital terminal with read-only Mt5ArchBridge writing account.json."
        : `ALPHA_MT5_PASSWORD for login ${ALPHACAPITAL_EXPECTED_LOGIN}.`,
  };
}

export function alphacapitalAccountSnapshot(report: AlphaCapitalLiveReport) {
  return {
