...
export function AlphaCapitalLiveCopy() {
  ...
  const hint = useMemo(() => {
    if (armed) {
      return `Armed. Each master fill copies to Alpha Capital 2765247 as a ${defaultLotsForFirm("alphacapital")}-lot EURUSD order of the same type.`;
    }
  ...
  return (
    <section className="space-y-3 rounded-lg border border-amber-500/30 bg-amber-500/5 p-3">
      ...
      <label className="flex items-start gap-2 text-xs text-amber-100/90">
        ...
        />
        <span>
          Copy each master fill to live Alpha Capital 2765247 as a {defaultLotsForFirm("alphacapital")}-lot
