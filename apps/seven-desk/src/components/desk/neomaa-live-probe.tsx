...
export function NeomaaLiveProbe() {
  ...
  const run = useCallback(async (poll = false) => {
    ...
    setError(null);
    try {
      const response = await fetch(poll ? "/api/neomaa/probe?poll=1" : "/api/neomaa/probe", {
        cache: "no-store",
      });
      if (!response.ok) {
  ...
  updateAccount(ACCOUNT_IDS.neomaa, {
    ...
    ...(report.connectionStatus === "disconnected"
      ? {
          ...
          statusReason:
            "Neomaaa-global trade server offline (weekend FX). File-bridge is writing. Restart this book only — do not restart other books.",
        }
      : {}),
