...
string SidecarPath(const string sym)
  {
   return "mt5_arch\\htf_fib_effective_" + sym + ".txt";
  }

// Refuse a leftover sidecar from a previous chart/symbol/TF. iCustom OnInit
// must recreate it for *this* run before we pin left/right/source/version.
bool DeleteLeftoverSidecar(const string path)
  {
   if(!FileIsExist(path))
      return true;
   if(!FileDelete(path))
     {
      Print("ExportHtfFibParity: FAIL cannot delete leftover sidecar ",
            path, " err=", GetLastError());
      return false;
     }
   if(FileIsExist(path))
     {
      Print("ExportHtfFibParity: FAIL leftover sidecar still present ", path);
      return false;
     }
   return true;
  }
...
bool ReadEffectiveConfig(const string sym, const string expect_tf, string &ver,
                         string &got_sym, string &got_tf,
                         int &left, int &right, int &fib_src)
  {
   string path = SidecarPath(sym);
   ...
   if(got_sym != sym)
     {
      Print("ExportHtfFibParity: FAIL sidecar symbol=", got_sym, " != ", sym);
      return false;
     }
   if(got_tf != expect_tf)
     {
      Print("ExportHtfFibParity: FAIL sidecar chart_tf=", got_tf,
            " != ", expect_tf);
      return false;
     }
   return (StringLen(ver) > 0 && left > 0 && right > 0 &&
           (fib_src == 0 || fib_src == 1));
  }
...
void OnStart()
  {
   ...
   // Delete any leftover sidecar first so a stale other-symbol/TF/version
   // file cannot pin this dump. iCustom OnInit must recreate it.
   string sidecar = SidecarPath(sym);
   if(!DeleteLeftoverSidecar(sidecar))
      return;
   ...
   waited = 0;
   while(!FileIsExist(sidecar) && waited < InpWaitMs)
     {
      Sleep(200);
      waited += 200;
     }
   if(!FileIsExist(sidecar))
     {
      Print("ExportHtfFibParity: FAIL sidecar not recreated after iCustom ",
            sidecar);
      IndicatorRelease(handle);
      return;
     }
   ...
   bool cfg_ok = ReadEffectiveConfig(sym, TfName(chart_tf), ind_ver,
                                    ind_sym, ind_tf,
                                    ind_left, ind_right, ind_src);
   if(!cfg_ok)
     {
      Print("ExportHtfFibParity: FAIL indicator sidecar unusable ", sidecar);
      IndicatorRelease(handle);
      return;
     }
   ...
   FileCopy(sidecar, 0, dir + "\\htf_fib_effective.txt", FILE_REWRITE);
   ...
   FileWriteString(hm, "  \"indicator_config_ok\": true,\n");
   FileWriteString(hm, "  \"indicator_version\": \"" + JsonEsc(ind_ver) + "\",\n");
   FileWriteString(hm, "  \"indicator_symbol\": \"" + JsonEsc(ind_sym) + "\",\n");
   FileWriteString(hm, "  \"indicator_chart_tf\": \"" + JsonEsc(ind_tf) + "\",\n");
