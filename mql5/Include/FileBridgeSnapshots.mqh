//+------------------------------------------------------------------+
//| FileBridgeSnapshots.mqh — heartbeat + JSON snapshots only.       |
//| Used by Mt5ArchBridge and Mt5ArchBridgeReadOnly.                 |
//| Snapshot writes only. Never reads desk order-bridge request files. |
//+------------------------------------------------------------------+
#ifndef FILE_BRIDGE_SNAPSHOTS_MQH
      ...
      if(now - s_last_err >= 60)
        {
         ...
         Print("Write fail ", rel, " err=", GetLastError(),
               " — remove EXTRA file-bridge EA from other charts");
        }
      return;
     }
...
void WriteAccount()
  {
   ...
   if(!connected && (now - s_last_diag) >= 30)
     {
      s_last_diag = now;
      Print("file-bridge account offline-ish login=", login,
...
void WriteAll()
  {
   ...
   Put(g_dir + "\\heartbeat.txt",
       ...
       " readonly=" + (BRIDGE_READONLY ? "true" : "false"));
  }
