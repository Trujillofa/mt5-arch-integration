...
|------|------|---------------|-------------------|
| `Indicators/ForexHtfPivotsFib.mq5` | `ForexHtfPivotsFib.pine` | **8** | No `forex_sr_levels.csv` / `.tpl` S/R import. No `htf_fib_effective_*.txt` writer. No tester H1 fallback when HTF history is empty. Pivots via `ta.pivothigh` + `request.security(..., gaps_on, lookahead_off)` ≈ confirm-bar close (not center). Wine `Comment` / `ChartObjects` → table + plots. Fib source Daily-fallbacks when chart TF > H4 (MQL5/`use4h`); `UniversalOverlay` does not — it keeps H4 `request.security("240")`. |
| `Indicators/ForexIndicatorTemplate.mq5` | `ForexIndicatorTemplate.pine` | **9** (buffer **8** is the short arrow, not the signal) | Session hours are broker **SERVER** time in MQL5. Pine default is `syminfo.timezone` (`InpSessionTimezone=exchange`); any valid IANA is accepted (invalid names fail on TradingView). Live bid/ask spread gate is stubbed (historical TV has no MT5 spread). Prior-day H/L/O uses completed exchange `D` via `[1]` + `lookahead_on`. |
| `Indicators/UsIndexSessionScalp.mq5` | `UsIndexSessionScalp.pine` | **8** | DST is **not dropped**: IANA `America/New_York` / `Europe/London` / `Asia/Tokyo` replaces `IndexSessionUtils.mqh` server-offset + US/UK DST tables. `InpServerUtcOffsetHours` unused. No `IdxSpreadPoints` on history. No `IndexM5Export`. Session boxes / GDI vlines → bgcolor + plotshape. Volume is TV `volume` vs MT5 `tick_volume`. Frozen default family still ORB. `InpShowVwap=false` empties signal VWAP (ORB / VWAP_BOUNCE fail `na`), matching MQL5 `BufVwap`. `InpDrawDays` is an ET-date plot cutoff (not MQL5 `rates_total` bars). |
| `Indicators/BtcTrendPullback.mq5` | `BtcTrendPullback.pine` | **7** | HTF bias uses `request.security(..., [1], lookahead_on)` = completed HTF bar (same idea as `CompletedHtfShift`). No session clock. Rolling VWAP uses chart volume, not tick volume. |
...
| FX template | Broker server hours | `InpSessionTimezone` (`exchange` = `syminfo.timezone`, or any IANA) |
...
|---------------|------------|---------------|-------|
| FX HTF Fib (ForexHtfPivotsFib) | `ForexHtfPivotsFib.pine` | **8** | Confirm-bar HTF pivots + golden-zone RSI. S/R CSV stays a stub. Wrapper keeps H4 fib source on TF>H4 (`fibFrom4h = fibSrc == 0`); standalone/MQL5 Daily-fallback. Prev-bar edge uses `var` fib/dir snapshots. |
| FX template (ForexIndicatorTemplate) | `ForexIndicatorTemplate.pine` | **9** | Buffer 8 is the short arrow, not the signal. Spread gate stubbed. Completed D PDH via `[1]`+`lookahead_on`. Session TZ is free-form IANA (`exchange` default). Fast/slow/bias EMAs also honor `InpShowEmas`. |
| US index scalp (UsIndexSessionScalp) | `UsIndexSessionScalp.pine` | **8** | IANA NY / London / Tokyo DST clocks. VWAP display-off empties signal VWAP (ORB / VWAP_BOUNCE fail `na`). `InpDrawDays` ET-date plot cutoff. |
